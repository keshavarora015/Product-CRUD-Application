...
public class ProductDaoImpl implements ProductDao {
	...
	public void updateProduct(Product product) {
		try {
			PreparedStatement pstmt=connection.prepareStatement("update productcart set price=?,quantity=? where id=?");
	...
	public Product getProductById(int id) {
		...
		try {
			...
			ResultSet rs = pstmt.getResultSet();
			if (rs.next()) {
				tempProduct = new Product(rs.getInt("id"), rs.getString("name"), rs.getInt("price"),
						rs.getInt("quantity"));
			}
			if (tempProduct != null) {
				return tempProduct;
